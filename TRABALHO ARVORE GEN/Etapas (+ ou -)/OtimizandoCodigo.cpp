...
void imprimirArvore(shared_ptr<Pessoa> pessoa, int geracao, bool ascendentes, ofstream& arquivo) {
	...
	if(ascendentes == true) {
		for(auto& filho : pessoa->ascendentes) {
			imprimirArvore(filho, geracao + 1, true, arquivo);
		}
		if(geracao != 0) {
...
// conta todos os relacionados da pessoa passada (recursivo, com controle de visitados)
	...
	if(ascendentes == true) {
		for (auto& pai : pessoa->ascendentes) {
			...
			if (visit.find(pai->name) == visit.end()) {
				...
				totalAsc++;                        // conta este ascendente
				totalAsc =+ contarRelacionados(pai, visit, totalAsc, true); // conta ascendentes recursivamente
			}
		}
		return totalAsc;
	} else {
		for (auto& filho : pessoa->descendentes) {
			...
			if (visit.find(filho->name) == visit.end()) {
				...
				totalDesc++;                        // conta este filho
				totalDesc =+ contarRelacionados(filho, visit, totalDesc, false); // conta descendentes recursivamente
			}
		}
		return totalDesc;
	}
...
// conta os relacionados diretos da pessoa passada (recursivo, com controle de visitados)
	...
	if(ascendentes == true) {
		for (auto& pai : pessoa->ascendentes) {
			...
			if (visit.find(pai->name) == visit.end()) {
				visit.insert(pai->name);           // marca como visitado			
				totalAsc++;                        // conta este ascendente
				contarRelacionados(pai, visit, totalAsc, true); // conta ascendentes recursivamente
			}
		}
		return totalAsc;
	} else {
		for (auto& filho : pessoa->descendentes) {
			...
			if (visit.find(filho->name) == visit.end()) {
				visit.insert(filho->name);         // marca como visitado
				totalDesc++;                        // conta este filho
				contarRelacionados(filho, visit, totalDesc, false); // conta descendentes recursivamente
			}
		}
		return totalDesc;
	}
...
void calcularDistanciaParentesco(vector<shared_ptr<Pessoa>>& pessoas, string pessoa1, string pessoa2, ofstream& arquivo) {
    ...
    if (distancia == -1) {
        arquivo << "\n" << "Nao existe relacao entre " << pessoa1 << " e " << pessoa2 << "." << "\n";
    } else {
        arquivo << "\n" << pessoa1 << " e " << pessoa2 << " sao parentes de " << distancia << "o grau." << "\n";
	}
}
